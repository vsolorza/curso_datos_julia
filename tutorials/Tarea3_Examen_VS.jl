...
# ╔═╡ 919be0be-071a-4bdd-8062-2a1623a8cb8b
using Distributions,Plots, PlutoUI, Unitful, StatsPlots, DataFrames, LinearAlgebra, Latexify, MAT, Dates, Statistics, Random

# ╔═╡ 22891e7b-141e-4f32-baa5-cdd72c43ce92
using CairoMakie

# ╔═╡ e2474450-d134-11ee-2f26-cb4430cc5b38
md"""
# Análisis de datos - Tarea 3 / Examen
## Victor Daniel Solorza Rodríguez
---
"""

# ╔═╡ 8e115d43-d1d5-4ea4-af8b-abc96584c7b8
md"""
1.  Calculen la correlación entre las series de promedios diarios de velocidad v (meridional)del ADCP de Puerto Morelos v a nivel 18, (v[:,18]) con la de los  promedios diarios en los niveles (a) v[:,19] y (b) v[:,10]
2.   a) Utilicen el método Boostrap para generar 10^4 réplicas de cada uno de los datos  v[:,19] y  v[:,10] calculen la correlación entre v[:,18]  y cada una de las réplicas con lo que obtendrán 10^4 correlaciones de v[:,18] con v[:,19] y 10^4 con v[:,10].
     b)  Obtengan los histogramas  de la distribución de las correlaciones
     c) Calculen el intervalo de confianza de 95% de estas correlaciones usando los histogramas "normalizados" (integral del histograma debe ser 1) y hagan una gráfica como la que se muestra en el script en el ejemplo de Bootstrap.
3.  PARA CADA NIVEL: [u[:,18],v[:,18]], [u[:,10],v[:,10]], [u[:,19],v[:,19]] 
     a) Calcular los ejes principales de la elipses de variabilidad del vector velocidad (Deben calcular eigenvalores y eigenvectores de las matrices de covarianza (2x2) entre las componentes del vector velocidad.      
     b)  Hacer Scatter plots de [u,v] y grafiquen los eigenvectores (son unitarios, los pueden multiplicar por la raíz cuadrada de la magnitud de cada eigenvalor que es una medida de la desviación estándar en esa dirección).    
     c)  Proyectar vector velocidad (producto punto! de [u,v] sobre cada eigenvector de manera que obtienen una serie de tiempo de la velocidad a lo largo de cada "eje" (eigenvector).    
     d) Calcular la correlación entre las  proyecciones a lo largo de esos ejes. Hacer un Scatter plot de vector velocidad entre las componentes a lo largo de esos ejes.
"""

# ╔═╡ 2c1cced6-ea50-47ff-b430-36080e7f18ba
begin
...
C18=	[std(udn18)*std(udn18) cov(udn18,vdn18);
		cov(udn18,vdn18) std(vdn18)*std(vdn18)]

C19=	[std(udn19)*std(udn19) cov(udn19,vdn19);
		cov(udn19,vdn19) std(vdn19)*std(vdn19)]

C10=	[std(udn10)*std(udn10) cov(udn10,vdn10);
		cov(udn10,vdn10) std(vdn10)*std(vdn10)]

# Promedio de las velocidades diárias normalizadas
μv18=mean(vdn18)
μv19=mean(vdn19)
μv10=mean(vdn10)
...
axislegend(ax1)
	for i in 1:651
	CairoMakie.scatter!(ax1, (udn18[i], vdn18[i]); color=(:red, 0.5))
	end
f1
...
axislegend(ax2)
	for i in 1:651
	CairoMakie.scatter!(ax2, (udn19[i], vdn19[i]); color=(:red, 0.5))
	end
f2
...
axislegend(ax3)
	for i in 1:651
	CairoMakie.scatter!(ax3, (udn10[i], vdn10[i]); color=(:red, 0.5))
	end
f3
