...
using DifferentialEquations, Plots, Measurements, PlutoUI
...
# ╔═╡ 6d16733a-18a0-4bd8-95c4-d216bc00dda8
md"""
# $title
"""
...
Modificación de ejemplo del Template de Curso Pluto 
[Template](https://rikhuijzer.github.io/JuliaTutorialsTemplate)(<https://rikhuijzer.github.io/JuliaTutorialsTemplate>) 
tomando ejemplos de [SciML](https://github.com/SciML/) (<https://tutorials.sciml.ai/html/models/01-classical_physics>).

Primero  tomamos el caso del Oscilador Armónico con errores en las condiciones iniciale descrito por la ecuación diferencial de segundo orden
...
```math
A = \sqrt{c_1^2 + c_2^2} \: \: \: \: \: \text{and} \: \: \: \: \: \tan \phi = \frac{c_2}{c_1}
```

Con $c_1$, $c_2$ constantes determinadas por las condiciones iniciales tales que
$c_1$ es la posición inicial, $\omega c_2$ la velocidad inicial.

Aunque se podría usar  `SecondOrderODEProblem` de DifferentialEquations.jl
Se usa un sistema de primer orden `ODEProblem` para resolver las eccuaciones
porque no pudu utilizarse Measurements.jl  en la versión de segundo orden. 
"""
...
md" y las siguientes condiciones iniciales:"
...
# ╔═╡ ea93e2e2-90f7-4249-8357-4d87205af899
tspan1

# ╔═╡ 5d486c91-d950-4304-81df-63540b23df68
md"La función para el problem de segundo orden es:"
...
# ╔═╡ c6537308-7c08-4811-9d9a-2a52e1c94325
md"Pero no funciona con Measurements.jl,Entonces definimos el sistema de primer orden"

# ╔═╡ b4285326-a2cc-4826-b3ff-33e8f0bc9bfc
md"""
```math
\begin{eqnarray}
\dot{x} &= v \\
...
# ╔═╡ 19a64fc9-088f-49a8-a034-9bdbf3475041
function harmonicoscillator2!(du2, u2, ω, t)
du2[1] = u2[2]
du2[2]= -ω^2 * u2[1]
end;

# ╔═╡ a0bbb00f-5fd8-4ce1-a5ad-1bfaf70b5d9d
prob1 = ODEProblem(harmonicoscillator2!, u2, tspan1, ω);

# ╔═╡ 46a440c2-6078-4884-959a-30359b552c0b
md"Pasamos esto a los `solvers`:"

# ╔═╡ 74e0bf5d-71db-4bf9-9e1b-49ddac38239e
sol1 = solve(prob1)

# ╔═╡ 38c44295-0490-4445-8ef7-ac089fae88ca
size(sol1.t)

# ╔═╡ 7debb004-045f-4a3e-a942-efe6eacab691
plot(sol1, linewidth = 2, plotdensity=100, xaxis = "t", title="Oscillator with ω=$ω", label = ["x [m]" "vel [m/s]"])
#layout = (2, 1))

# ╔═╡ 55a23e2b-3938-4a79-9219-9a59ee3350e9
A

# ╔═╡ ff355b90-5971-40dd-a661-d9197e3b75d0
begin
...
# ╔═╡ 4eeb5d35-50e1-47c6-a635-4f0e58cb20d5
size(sol1.u)

# ╔═╡ 005d59fa-2800-4402-8cd1-e22626644900
function lorenz!(dv, v, p, t)
    dv[1] = 10.0 * (v[2] - v[1])
    dv[2] = v[1] * (28.0 - v[3]) - v[2]
    dv[3] = v[1] * v[2] - (8 / 3) * v[3]
end

# ╔═╡ f200a765-28cb-49a6-a08b-77a9ea1a2913
...
# ╔═╡ 804e3545-61dd-4180-bd95-436ce144884d
function parameterized_lorenz2!(du, u, p, t)
    x, y, z = u
    σ, ρ, β = p
    du[1] = dx = σ * (y - x)
    du[2] = dy = x * (ρ - z) - y
...
# ╔═╡ 56098f56-5e62-4976-ab6b-f4e579011787
@bind σ Slider(1:20,default=10)

# ╔═╡ 0395e10c-200b-4aff-85d0-9886e7d482a4
@bind ρ Slider(2:2:50,default=28)

# ╔═╡ 10746f76-4405-47b7-98d0-f12ae79e12c8
@bind β Slider(1//3:1//3:16//3,default=8//3)

# ╔═╡ 85cd6b02-a73f-4bfa-8afd-ca002eb8ad91
begin
...
# ╔═╡ 1d0973a8-5f28-423f-a7c8-64a9e486ed2a
begin
...
# ╔═╡ 9101554e-b7b2-4826-b68a-7fa77ee5b6ae
begin
...
#sol_lorenz(tt)
#Measurements.value.([sol_lorenz.u[i][1] for i=1:length(sol_lorenz.t)])

# ╔═╡ 8e525081-6aab-4bfd-907e-b87647612075
plot(sol_lorenz.t[300:400],Measurements.value.(xlorenz[300:400]),ribbon=Measurements.uncertainty.(xlorenz[300:400]),ylimits=(-20.0,20.0))#idxs = (0,1))#layout=(3,1))#idxs = (0,1))
#Measurements.value.(xlorenz)

# ╔═╡ 6adef560-44d2-4a94-b4e2-dd2561204007
plot(Measurements.value.(xlorenz), Measurements.value.(ylorenz), Measurements.value.(zlorenz),grid=true)

# ╔═╡ fd6a9d00-7c6d-433c-9f6a-ef0a1257e766
begin
...
# ╔═╡ e9493845-ebb2-4933-be8c-dddf047bfc97
length(Measurements.uncertainty.(sol.u))

# ╔═╡ ecc20690-5be1-409c-8ae4-59c5eb176fb1
#plot!(sol.t,sol)
...
# ╔═╡ 8149de8a-4530-4da7-bfb6-9c0a5491505b
Measurements.uncertainty.(sol.u)
